...
- **Multi-user auth (open item)**: replaced the single shared `ADMIN_TOKEN` with a JWT in an HttpOnly `SameSite=Lax` cookie (`backend/app/auth.py`), Argon2 password hashing (`pwdlib[argon2]`), and a single admin account provisioned on first login from `ADMIN_EMAIL`/`ADMIN_PASSWORD` env vars (`backend/app/routers/auth.py`). No forced password change — login lands straight in the app; the admin can change their password anytime from Settings (Security). Endpoints: `POST /api/v1/auth/login`, `POST /api/v1/auth/logout`, `POST /api/v1/auth/change-password`, `GET /api/v1/auth/me`. Login returns 503 when env creds are unconfigured. Sessions last 30 days (`JWT_EXPIRY_MINUTES=43200`) and are invalidated on password change via the `pwd` claim. Regression tests: `tests/test_auth.py`.
