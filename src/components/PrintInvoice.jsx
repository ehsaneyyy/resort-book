    ...
    <div className="print-overlay fixed inset-0 z-[9999] bg-white overflow-auto" style={{ display: 'none' }}>
      <div className="max-w-lg mx-auto p-10 text-[#1a1a2e] font-sans">
        ...
        <div className="mb-4">
          ...
          <div className="flex justify-between py-0.5 text-[12px]"><span className="text-[#888]">Check-in</span><span className="font-medium">{formatDate(booking.checkIn)} at {formatTime(resort.checkInTime) || '2:00 PM'}</span></div>
          <div className="flex justify-between py-0.5 text-[12px]"><span className="text-[#888]">Check-out</span><span className="font-medium">{formatDate(booking.checkOut)} at {formatTime(resort.checkOutTime) || '11:00 AM'}</span></div>
