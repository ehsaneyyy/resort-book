          ...
          <div key={g.id} className="bg-dark-800/50 rounded-lg border border-white/[0.02] hover:border-white/[0.05] transition-colors p-4 cursor-pointer" onClick={() => setDetail(g)}>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-slate-600">{g.id}</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button onClick={(e) => { e.stopPropagation(); const link = whatsappLink(g.phone, `Hi ${g.name}!`); if (link !== '#') window.open(link, '_blank'); }} className="p-2 text-slate-600 hover:text-emerald-400 transition-colors"><MessageCircle className="w-4 h-4" /></button>
      ...
      {detail && (
        <Modal title={detail.name} onClose={() => setDetail(null)}>
          <div className="space-y-5">
            <div>
              <h4 className="text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-2.5">Contact</h4>
            ...
            <div>
              <h4 className="text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-2.5">Stay History</h4>
