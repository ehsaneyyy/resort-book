  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openAdd} className="px-3 py-1.5 bg-amber-500/10 text-amber-400 text-[12px] font-medium rounded hover:bg-amber-500/20 transition-colors flex items-center gap-1.5">
              ...
              <div className="p-5">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-600">{r.type} · Floor {r.floor}</p>
                    </div>
                  </div>
                  <span className={`px-2 py-0.5 text-[10px] font-medium rounded ${isOcc ? 'bg-red-500/8 text-red-400/70' : 'bg-emerald-500/8 text-emerald-400/70'}`}>{isOcc ? 'Occupied' : 'Available'}</span>
                </div>

                <p className="text-[13px] text-slate-500 mb-4 leading-relaxed">{r.description}</p>

                <div className="flex flex-wrap gap-1 mb-4">
                  {r.amenities.map(a => <span key={a} className="px-2 py-0.5 bg-dark-700/50 rounded text-[10px] text-slate-600">{a}</span>)}
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-white/[0.02]">
                  <div>
                    ...
                    <span className="text-[11px] text-slate-600">/night</span>
                    {r.weekendPrice !== r.price && <span className="text-[11px] text-slate-600 ml-2">Wknd {formatCurrency(r.weekendPrice)}</span>}
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => openEdit(r)} className="p-1.5 text-slate-600 hover:text-amber-400 transition-colors"><Edit3 className="w-3.5 h-3.5" /></button>
