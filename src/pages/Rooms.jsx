  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-[12px] text-slate-600">{rooms.length} rooms · {occupied.length} occupied</p>
        <button onClick={openAdd} className="px-3 py-2 bg-amber-500/10 text-amber-400 text-[12px] font-medium rounded hover:bg-amber-500/20 transition-colors flex items-center gap-1.5">
      ...
      <div className="flex gap-1 overflow-x-auto pb-1">
        {types.map(t => (
          <button key={t} onClick={() => setFilter(t)} className={`px-3 py-2 rounded text-[12px] font-medium transition-colors whitespace-nowrap ${filter === t ? 'text-white' : 'text-slate-600 hover:text-slate-400'}`}>{t === 'all' ? 'All' : t}</button>
        ))}
      </div>
              ...
              <div className="p-5">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[12px] text-slate-600">{r.type} · Floor {r.floor}</p>
                    </div>
                  </div>
                  <span className={`px-2 py-0.5 text-[12px] font-medium rounded ${isOcc ? 'bg-red-500/8 text-red-400/70' : 'bg-emerald-500/8 text-emerald-400/70'}`}>{isOcc ? 'Occupied' : 'Available'}</span>
                </div>

                <p className="text-[13px] text-slate-500 mb-4 leading-relaxed">{r.description}</p>

                <div className="flex flex-wrap gap-1 mb-4">
                  {r.amenities.map(a => <span key={a} className="px-2 py-0.5 bg-dark-700/50 rounded text-[12px] text-slate-600">{a}</span>)}
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-white/[0.02]">
                  <div>
                    ...
                    <span className="text-[12px] text-slate-600">/night</span>
                    {r.weekendPrice !== r.price && <span className="text-[12px] text-slate-600 ml-2">Wknd {formatCurrency(r.weekendPrice)}</span>}
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => openEdit(r)} className="p-2 text-slate-600 hover:text-amber-400 transition-colors"><Edit3 className="w-4 h-4" /></button>
                    <button onClick={() => del(r.id)} className="p-2 text-slate-600 hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <Modal title={modal === 'edit' ? 'Edit Room' : 'Add Room'} onClose={() => setModal(null)}>
          <form onSubmit={save} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className="block text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-1.5">Type</label><select value={form.type || ''} onChange={e => setForm({ ...form, type: e.target.value })} className="w-full px-3 py-2 bg-dark-700 border border-white/[0.03] rounded text-white text-sm focus:outline-none focus:border-white/10"><option>Standard</option><option>Deluxe</option><option>Suite</option><option>Premium Suite</option><option>Villa</option></select></div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div><label className="block text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-1.5">Weekday (₹)</label><input required type="number" value={form.price || ''} onChange={e => setForm({ ...form, price: e.target.value })} className="w-full px-3 py-2 bg-dark-700 border border-white/[0.03] rounded text-white text-sm focus:outline-none focus:border-white/10" /></div>
              <div><label className="block text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-1.5">Weekend (₹)</label><input required type="number" value={form.weekendPrice || ''} onChange={e => setForm({ ...form, weekendPrice: e.target.value })} className="w-full px-3 py-2 bg-dark-700 border border-white/[0.03] rounded text-white text-sm focus:outline-none focus:border-white/10" /></div>
              <div><label className="block text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-1.5">Capacity</label><input required type="number" value={form.capacity || ''} onChange={e => setForm({ ...form, capacity: e.target.value })} className="w-full px-3 py-2 bg-dark-700 border border-white/[0.03] rounded text-white text-sm focus:outline-none focus:border-white/10" /></div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div><label className="block text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-1.5">Beds</label><input required value={form.beds || ''} onChange={e => setForm({ ...form, beds: e.target.value })} className="w-full px-3 py-2 bg-dark-700 border border-white/[0.03] rounded text-white text-sm focus:outline-none focus:border-white/10" /></div>
              <div><label className="block text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-1.5">Size (sqft)</label><input required type="number" value={form.size || ''} onChange={e => setForm({ ...form, size: e.target.value })} className="w-full px-3 py-2 bg-dark-700 border border-white/[0.03] rounded text-white text-sm focus:outline-none focus:border-white/10" /></div>
              <div><label className="block text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-1.5">Floor</label><input required type="number" value={form.floor ?? ''} onChange={e => setForm({ ...form, floor: e.target.value })} className="w-full px-3 py-2 bg-dark-700 border border-white/[0.03] rounded text-white text-sm focus:outline-none focus:border-white/10" /></div>
            </div>
            <div><label className="block text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-1.5">Description</label><textarea rows={2} value={form.description || ''} onChange={e => setForm({ ...form, description: e.target.value })} className="w-full px-3 py-2 bg-dark-700 border border-white/[0.03] rounded text-white text-sm focus:outline-none focus:border-white/10 resize-none" /></div>
            <div><label className="block text-[12px] font-semibold text-slate-500 uppercase tracking-[1.5px] mb-1.5">Amenities (comma separated)</label><input value={form.amenities || ''} onChange={e => setForm({ ...form, amenities: e.target.value })} className="w-full px-3 py-2 bg-dark-700 border border-white/[0.03] rounded text-white text-sm focus:outline-none focus:border-white/10" placeholder="AC, WiFi, TV..." /></div>
            <div className="flex gap-2 pt-2">
              <button type="submit" className="flex-1 py-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 text-[12px] font-medium rounded transition-colors">{modal === 'edit' ? 'Update Room' : 'Add Room'}</button>
